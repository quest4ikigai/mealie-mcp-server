...
`categories`/`tags` follow the same name/slug/ID matching convention as `get_recipes`. They're passed straight through to Mealie for the food-filter and text-search paths; for the Recipe Finder path (which has no taxonomy filters of its own) they're applied to the returned candidates instead.
