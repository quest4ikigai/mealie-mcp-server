...
A [Model Context Protocol (MCP)](https://modelcontextprotocol.io) server for [Mealie](https://github.com/mealie-recipes/mealie) recipe management. Exposes 51 tools and 1 prompt for AI assistants to search, create, and manage recipes, meal plans, shopping lists, categories, tags, and foods.
...
See [API Coverage](./API_COVERAGE.md) for a detailed breakdown of all 51 tools and their corresponding Mealie API endpoints.
...
This tool sends a `PATCH /api/recipes/{slug}` body containing **only** `{ "recipeIngredient": [...] }` — never a full recipe object, and never a `GET` first. That's what makes every unrelated *scalar* field (name, description, settings, nutrition, categories/tags, etc.) structurally safe: Mealie's `PATCH` route merges only the keys present in the request onto the existing recipe, so fields we never mention can't be affected by anything we do.

**Known upstream Mealie limitation — `recipeInstructions[].id` is regenerated on every recipe update, PATCH or PUT, no matter what.** This was confirmed by live testing against a real Mealie instance, including explicitly echoing `recipeInstructions` back in the request body with its exact current IDs — the IDs still changed. Root cause traced into Mealie's own source: both `PUT` and `PATCH` ultimately call the same repository `update()`, which re-invokes the SQLAlchemy model's constructor on the already-persisted recipe; `recipe_instructions` is declared with `cascade="all, delete-orphan"`, so every instruction row gets deleted and recreated with a fresh ID as a side effect of that re-construction. This is not something this tool causes or can avoid — the same thing would happen from any client, including Mealie's own web UI, on any recipe save. Instruction **content** (text, title, summary, ingredient references) is preserved correctly; only the IDs churn. If your workflow depends on stable instruction IDs across ingredient updates, treat that as currently unsupported by Mealie itself.
...
### Recipes (14)
`get_recipes`, `get_recipe_detailed`, `get_recipe_concise`, `get_recipes_batch`, `get_recipes_detailed_batch`, `create_recipe`, `patch_recipe`, `update_recipe_ingredients`, `update_recipe_taxonomy`, `update_recipe_taxonomy_batch`, `duplicate_recipe`, `mark_recipe_last_made`, `set_recipe_image_from_url`, `delete_recipe`
